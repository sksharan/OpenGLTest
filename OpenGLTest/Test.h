...
/* Generates the Stanford dragon model from an obj file tht needs to be deleted by the user. */
OBJObject* genDragon();

/* Generates a heightmap object that needs to be deleted by the user. Starts at the designated 'start_loc' */
HeightmapObject* genHeightmapObject(glm::vec3 start_loc);

/* Generates a heightmap whose height values are determined by Perlin noise equations. Starts at the designated 'start_loc' */
